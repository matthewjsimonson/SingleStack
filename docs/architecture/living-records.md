...
Status: **partially built** (narrative side) · **metric side proposed** · Owner: SingleStack
Surfaced during dogfood (finding #5). No new implementation yet — this scopes the
direction so the metric side is built deliberately, not half-started.
...
## Proposed shape (metric side) — for review, not yet built
