...
| 9 | 3 | `/competitive` | BUG | P1 | No way to delete a competitor record (GovDash mistakenly tracked, couldn't remove). RLS already allowed delete (`for all`); UI never exposed it. Added a confirm-guarded `×` remove button per competitor row. | fixed |
