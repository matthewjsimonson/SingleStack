...
| 5 | 1 | records (architecture) | SIGNAL | P0-concept | Records must be LIVING: dynamic with signals/releases, human-ratified on a controlled cadence — esp. customer metrics (NPS, usage) shown month-over-month. Found: narrative side ALREADY built (`field_revisions` + `accept_proposal()` = signals→proposal→ratify→record moves, with history). Gap = METRIC fields (numeric, time-series MoM, signal-bound) don't exist; every field is `value text`. Scoped in `docs/architecture/living-records.md` — design only, not built. Needs decision before build. | flagged — design doc |
