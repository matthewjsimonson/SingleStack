...
| 14 | infra | dev DB drift | BUG | **P0 infra** | THE root cause of the "save doesn't work" saga: record save actually succeeded (`updates:1`) but the RELOAD query asked for `field_kind`/`metric_unit` → 400, because the dev DB was missing migrations. Why: Vercel auto-deploys app code from our feature branch, but the migration workflow only triggers on push to `develop`/`main` — so code ran ahead of the DB. Resolved: ran Deploy-to-Supabase on dev from our branch → all 6 pending migrations applied (incl. P0 org fix + living_record_metrics). Confirmed via run logs. Also stabilized SectionedFields `target` prop (memo) + added a no-op-save guard. PIPELINE FIX: merging our branch → `develop` so code+migrations deploy in lockstep going forward. | fixed |

| 15 | 1 | product record → Modules | UX+BUG | P1 | (a) Module section only had a name box — now captures **name + "What it does" (description)**, using the modules.description column that already existed (no migration). Description renders under each module. (b) Module add was swallowing errors via `instanceof Error` (same masking bug as #12) — now surfaces the real PostgrestError. DB path verified on Postgres (name-only + name+description both insert under RLS). The earlier 'not saving' was the dev-DB-drift (finding #14), now resolved by applied migrations. |
| 16 | 1 | product record → Modules save | BUG | P1 | THE actual modules 'not saving' cause (distinct from #15's structure work): `load()` ordered modules by `.order("position")`, but the **modules table has no `position` column** → the reload SELECT 400'd → zero modules shown after a successful insert. Latent since inception (modules never displayed; only surfaced now that one was added). Fixed: order by `created_at` only, and surface load errors so a future reload failure isn't silent. Swept all 10 `.order("position")` calls — the other 8 tables (competitors, capabilities, battlecard_items, initiatives, signal_themes, decision_options, initiative_fields, record_fields) all HAVE the column; modules was the only broken one. |
---
