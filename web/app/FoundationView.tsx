...
// Foundation overview — the big picture. Org-wide stats across the model, then
// each product as a rich card showing its structure (modules/features/GTM
// counts) and pending-proposal load, so you see the whole and drill in.
...
export default function FoundationView() {
  ...
  const load = useCallback(async () => {
    const [{ data: prods }, { data: mods }, { data: feats }, { data: gtm }, { data: props }, { count: sigCount }] = await Promise.all([
      ...
      supabase.from("proposals").select("id, product_id, gtm_record_id, status").eq("status", "pending"),
    ...
    (gtm ?? []).forEach((g) => { gtmProduct[g.id] = g.product_id; if (c[g.product_id]) c[g.product_id].gtm++; });
    (props ?? []).forEach((pr) => {
    ...
    setCounts(c);
    setTotals({ products: products.length, gtm: (gtm ?? []).length, signals: sigCount ?? 0, pending: (props ?? []).length });
    setLoading(false);
  }, [supabase]);
  ...
  return (
    <div>
      <PageHeader
        title="Foundation"
        meta="The canonical truth of your products — and every layer agents reason over."
        actions={mode === null ? (
          <>
      ...
      {!loading && products.length === 0 && mode === null && (
