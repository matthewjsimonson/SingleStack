...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const pol = await resolveModelPolicy(supabase, { task: "draft_decision", fallback: { model: MODEL, effort: "high" } });
    ...
    await logUsage(supabase, { task: "draft_decision", model: pol.model, usage: resp.usage });
