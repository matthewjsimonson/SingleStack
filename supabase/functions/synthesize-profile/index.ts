    ...
    fields: {
      ...
      items: {
        ...
        properties: {
          ...
          label: { type: "string" },
          value: { type: "string" },
          // Which focus of the signals profile this node sits on. The center is
          // 'core'; the three focuses are competitive/market/technology.
          // Competitor (battlecard) profiles set vector='core'.
          vector: { type: "string", enum: ["core", "competitive", "market", "technology"] },
          // Level of focus, by how direct the signal is: 3 = direct, 2 = adjacent,
          // 1 = indirect.
...
const VECTOR_INTERVIEW: Record<string, string> = {
  ...
  competitive: "Sharpen who we actually compete with and on what — COMPETING PRODUCTS specifically: what buyers pick instead when we lose (point tool / suite / DIY / nothing), the ONE capability we're put head-to-head on, adjacent tools newly creeping into our space, and where buyers compare tools like us. Ask what the records don't pin down about rivals and battlegrounds.",
  market: "Sharpen the applicable market as ONE picture — industries, personas, and the market news that moves them — graded by applicability: DIRECT (our market: the verticals we serve and the people who buy/use us), ADJACENT (nearby segments and roles), INDIRECT (general movement worth catching). Ask who signs vs who uses daily, which verticals are core ICP vs occasional, what regulation/budget cycles change, and where each segment's and persona's signal actually lives.",
  technology: "Sharpen the technology picture across THREE uses: what we EMBED in the product, what we BUILD the product with, and what powers our GTM — frontier AND open-source models across all three. Ask which are load-bearing, which frontier/open-source capability would most change what we can ship or how we sell, and what platform shifts threaten us.",
};
...
Deno.serve(async (req: Request) => {
  ...
  const VECTOR_KEYS = ["core", "competitive", "market", "technology"];
  ...
  try {
    ...
    if (scope === "competitor") {
      ...
      target = oneVector
        ? `the '${oneVector}' vector of our signals network — the records-grounded nodes that aim ${oneVector === "competitive" ? "the competitor search" : oneVector === "market" ? "market discovery (industries, personas, market news)" : oneVector === "technology" ? "technology watching (embedded, build, GTM — frontier and open-source)" : "the whole network"}`
      ...
      const VECTOR_AREAS: Record<string, string> = {
        ...
        competitive: "COMPETITIVE (competing PRODUCTS; weight = DIRECTNESS, and children INHERIT their parent's level — depth organizes, level classifies): 3 = DIRECT: rip-and-replace — a buyer could cancel us and move here. Cover BOTH kinds: PLATFORM rivals (compete across many of our capabilities at once) and POINT solutions that are direct on ONE capability we own — for each point archetype NAME the capability it attacks. 2 = ADJACENT: same job, different means — DIY/stitched stacks, services, in-house builds; nothing to rip out, same outcome. 1 = INDIRECT: different game, our world — tools sharing our budget line or buyers, larger players moving into our space, the voices defining our category. STRUCTURE: root each band with an ARCHETYPE node whose value states the band's test (e.g. the rip-and-replace test), then chain specific archetypes/instances beneath it (name specific companies ONLY with evidence). PLUS structural lenses: competitive_battlegrounds (where deals are won/lost, w3), differentiation (our wedge vs the alternative set, w3), competitive_search_focus (WHERE to look — review/comparison sites + the KIND of source, w3), win_loss_patterns (fills in once synthesized, w2), moves_to_watch (launches/pricing/funding/positioning shifts, w2).",
        market: "MARKET (the applicable market: industries + personas + market news; weight = APPLICABILITY; children inherit their parent's level): 3 = DIRECT: our market — the segments we actually serve (root the core segment; chain its sharpest sub-segments beneath), the ECONOMIC BUYER (root; chain the daily operators beneath), any second operator persona, and a demand-signals node (the hires/postings/behaviors that signal need for us). 2 = ADJACENT: nearby segments and roles that become direct as our motion spreads — root a 'one step out' archetype and chain them beneath. 1 = INDIRECT: the horizon — regulation and compliance forces on our buyers, budget/consolidation cycles, and the discourse reshaping our buyers' roles. Every node states WHERE its signal lives (the communities, boards, feeds, publications to watch) — read all of it from THIS org's records; never assume a market it doesn't serve. PLUS: market_search_focus (the aggregate where-to-look, w3), market_shifts_to_watch (w2).",
        technology: "TECHNOLOGY (weight = how load-bearing; children inherit their parent's level): 3 = DIRECT: what we EMBED in the product (root; chain the frameworks/protocols/fabrics it depends on beneath) and what we BUILD the product with (root; chain the capability step-changes to watch beneath). 2 = ADJACENT: one step out — alternative models/stacks including open-source (cost floor, self-host ceiling), the infrastructure we ride, and the technology powering our GTM motion. 1 = INDIRECT: the horizon — research not yet productized, standards/regulation shaping what we may ship, cost economics that move margins. Every node states what SHIFT would matter and where to watch for it — read the actual stack and dependencies from THIS org's records; never assume technologies it doesn't use. PLUS: technology_search_focus (w3), tech_threats_opportunities (w2).",
      };
    ...
    const system = [
      ...
      scope === "landscape"
        ? "BRANCHES: nodes CHAIN. When a node is a more specific facet of another node in the SAME vector (a sub-segment of a vertical, a specific rival under an archetype, a niche community under a persona), set its parent_key to that node's field_key. A child usually INHERITS its parent's weight — a named rival under a direct archetype is still direct; depth organizes, weight classifies. Drop the weight only when the child is genuinely less direct. Weight-3 nodes root the branches (parent_key=\"\"); don't chain deeper than 3 steps. Structural *_search_focus nodes root at the hub (parent_key=\"\")."
