  ...
  try {
    ...
    if (scope === "competitor") {
      ...
      const VECTOR_AREAS: Record<string, string> = {
        ...
        competitive: "COMPETITIVE (competing PRODUCTS): one node per competitor ARCHETYPE/attribute that pinpoints rivals — weight 3 = head-on/direct, 2 = adjacent/substitute, 1 = indirect/watching (name specific companies ONLY with evidence; chain a named rival off its archetype via parent_key). PLUS structural lenses: competitive_battlegrounds (where deals are won/lost, w3), differentiation (our wedge vs the alternative set, w3), status_quo_alternative (what buyers do instead today / 'do nothing', w2), competitive_search_focus (WHERE to look — review/comparison sites + the KIND of company/source, w3), win_loss_patterns (why we win/lose — fills in once synthesized, w2), moves_to_watch (rival launches/pricing/funding/positioning shifts, w2).",
        market: "MARKET (the applicable market: industries + personas + market news, ONE branch, graded by APPLICABILITY — weight 3 = direct/our market, 2 = adjacent, 1 = indirect/general): one node per VERTICAL we serve (w3 core ICP … w1 occasional; chain sub-segments off their parent vertical) AND one node per BUYER/USER persona (w3 economic buyer, w2 user/champion, w1 influencer; chain a persona's where-their-signal-lives node off it). PLUS: market_use_cases (how we apply per segment, w2), regulatory_landscape (compliance forces on the core segments, w2), market_objections (objections & buying triggers of the core personas, w2), market_search_focus (trade press, analyst coverage, the communities/forums/job boards where each segment's and persona's signal lives, w3), market_shifts_to_watch (market-size, regulation, budget, hiring, consolidation, w2).",
        technology: "TECHNOLOGY (three uses, each a BRANCH rooted at a w3 node — chain specifics off their use): tech_embedded (the models/platforms IN the product — what we ship on), tech_build (the tools/models we BUILD the product with), tech_gtm (the products/models that power our GTM). Cover frontier AND open-source models across all three. PLUS: platform_dependencies (ecosystems we ride & their shift risk, w2), frontier_capabilities_to_watch (what's newly possible — frontier or open-source — that changes what we can embed, build, or sell with, w3), technology_search_focus (model releases incl. open-source, research, provider changelogs/blogs, w3), tech_threats_opportunities (where model/platform shifts threaten or open our space, w2).",
      };
