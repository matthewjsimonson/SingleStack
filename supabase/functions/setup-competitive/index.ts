...
CHALLENGE THE PRODUCT CHARACTERIZATION above all else. If the records describe the product narrowly — one tidy category label — but its MODULES & FEATURES show it does materially more, then 'product', 'positioning', 'use_cases', and 'differentiation' are NOT covered: the label is hiding what the product actually is, and that mischaracterization is exactly why the search returns the wrong rivals (e.g. only 'product marketing' tools when the product also builds, sells, and learns). Characterize the product by EVERYTHING it actually does — every module, feature, and job — not the convenient category, and ask the questions that pin that down.
...
${RECORD_FIELD_CATALOG}`,
    ...
    if (step === "landscape") {
      ...
      const { text: briefing, usage } = await searchBriefing(
        supabase,
        key,
        `You are a competitive-landscape researcher. If the user NAMES known competitors in the market context, verify those FIRST (site, current positioning), then search beyond them. Use web search to identify the REAL competitors in the user's market — companies a buyer would actually evaluate against them. CHARACTERIZE THE USER'S PRODUCT BY EVERYTHING IT ACTUALLY DOES — its modules, features, and the full set of jobs across its whole scope — NOT a single tidy category label, which usually understates it and returns the wrong rivals (e.g. only 'product marketing' tools when the product also builds, sells, and learns); find the companies that compete on EACH major capability area, not just one category. Assess every candidate on FOUR dimensions: (1) buyer overlap — do they sell to the same personas? (2) industry overlap — same verticals? (3) capability overlap — which of the user's features/modules do they also offer? (4) positioning collision — do they claim the same category or replace the same thing? For each rival report: company name, homepage URL, head-on (direct) vs partial/adjacent, and the per-dimension read with what you found. For each rival also report their LinkedIn company page URL (linkedin.com/company/...) and a 2-3 sentence factual overview (who they are, what they sell, to whom) — URLs only from what you actually found, never constructed. Concrete and current. Return about ${target} rivals — the ones that genuinely matter MOST, not a directory dump; fewer high-overlap rivals beat a long thin list.`,
        [
    ...
    if (step === "competitors") {
      ...
      const briefing = (body.briefing as string | undefined)?.trim() || (await searchBriefing(
        supabase,
        key,
        `You are a competitive-landscape researcher. Use web search to identify the REAL competitors in the user's market — companies a buyer would actually evaluate against them. CHARACTERIZE THE USER'S PRODUCT BY EVERYTHING IT ACTUALLY DOES — its modules, features, and the full set of jobs across its whole scope — NOT a single tidy category label, which usually understates it and returns the wrong rivals (e.g. only 'product marketing' tools when the product also builds, sells, and learns); find the companies that compete on EACH major capability area, not just one category. Assess every candidate on FOUR dimensions: (1) buyer overlap — do they sell to the same personas? (2) industry overlap — same verticals? (3) capability overlap — which of the user's features/modules do they also offer? (4) positioning collision — do they claim the same category or replace the same thing? For each rival report: company name, homepage URL, head-on (direct) vs partial/adjacent, and the per-dimension read with what you found. For each rival also report their LinkedIn company page URL (linkedin.com/company/...) and a 2-3 sentence factual overview (who they are, what they sell, to whom) — URLs only from what you actually found, never constructed. Concrete and current. Return about ${target} rivals — the ones that genuinely matter MOST, not a directory dump; fewer high-overlap rivals beat a long thin list.`,
        [
          product.name ? `OUR PRODUCT: ${product.name}` : "",
          product.value_prop ? `VALUE PROP: ${product.value_prop}` : "",
          market ? `OUR MARKET (user's words): ${market}` : "",
